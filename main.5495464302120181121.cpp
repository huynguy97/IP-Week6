...
bool enter_filename (char filename [MAX_FILENAME_LENGTH])
{
    assert(filename); //                                        
    //                                                                                                
    cout << "Please enter a filename. The total number of characters cannot exceed " << MAX_FILENAME_LENGTH - 1 << ". \n" ; //                                    
    //                                                                                        
    ...
    {
        cout << "Your input name is too long." ;
        return false ;
    }
    else
    {
...
void next_generation (Cell now [ROWS][COLUMNS], Cell next [ROWS][COLUMNS])
{
    ...
    for (int row = 0 ; row < ROWS ; row++)
    {
        for (int column = 0; column < COLUMNS ; column++)
        {
            ...
            {
                int live_around = 0 ;
                live_around = now[row+1][column] + now[row-1][column] + now[row][column+1] + now[row][column-1] + now[row+1][column+1]
